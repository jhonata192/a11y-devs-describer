...
def render_pdf(document: dict[str, Any], output_path: Path, profile_name: str = "pdf", title: str | None = None) -> Path:
...
def _render_block(story, block: dict[str, Any], styles) -> None:
    ...
    if block_type == "heading":
        ...
        for row in block.get("rows", []):
            story.append(Paragraph(" | ".join(str(cell) for cell in row), styles["A11yBody"]))
